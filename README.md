...
1. **📥 输入网页数据** - 提供未分类的 URL 列表或 HTML 源码文件
2. **🔍 智能网页分类** - AI 自动识别并分组相同站点以及类似的网页布局（如：知乎推荐页/专栏页；CSDN博客页/社区页等）
3. **🎯 智能选取样例** - 从每个分类中自动选择代表性样例（通常 2-5 个）
4. **🧠 Schema 学习与代码迭代** - 多模态分析（HTML + 视觉）→ 提取数据模式 → 智能合并优化
5. **⚙️ 生成高质量解析器** - 输出可直接使用的 Python 代码
6. **⚡  自动高速批量处理** - 自动使用生成的解析器对该分类下所有网页进行批量解析
7. **📊 获得结构化数据** - 所有网页转换为统一的富含语义信息的结构化格式

> **注**：步骤 1～3 功能规划中，现阶段需手动设置需同类型网页目录。
